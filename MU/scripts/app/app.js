    ...
    var mu = global.mu,
        ...
        page_option = {
            ...
            afterPageTrans: function($oupage, $inpage, current) {
                ...
                if(hasTopic) {
                    idxTopic = $topic.closest('.page').index();
                    topicId = parseInt($topic.attr('data-topicId'));
                    if(!isInitTopic){
                        if(current === idxTopic){
                            isInitTopic = true;
                            ztUtil.getReplyList(topicId);
                        }
                    }else{
                        current === idxTopic ? scrollSubtitle.start() : scrollSubtitle.stop();
                    }
                }
            },
            /** 转场class对象 */
